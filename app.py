...
DecisionTree = pickle.load(open(models_path + 'LogisticRegression.pkl', 'rb'))
...
def predict_crop():
    try:
        ...
        # Prepare input data as a NumPy array for model prediction (for descision tree and others cant use for logistic regression)
        # X_new = np.array([[nitrogen, phosphorus, potassium, temperature, humidity, ph, rainfall]])

        #if we use logistic regression we need numerical input for model so convert the array into numeric values
        X_new = np.array([[float(nitrogen), float(phosphorus), float(potassium),float(temperature), float(humidity), float(ph), float(rainfall)]])
